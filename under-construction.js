(() => {
  ...
  function validWaNumber(value) {
    return String(value || '').replace(/\D/g, '').replace(/^0+/, '');
  }

  function waUrl(settings) {
    const number = validWaNumber(settings.whatsapp_number);
    if (!number) return '';
    return `https://wa.me/${number}?text=${encodeURIComponent(String(settings.whatsapp_message || ''))}`;
  }

  function renderPublic(settings) {
    document.documentElement.style.background = '#EBF3FF';
        ...
        <section style="width:min(980px,100%);background:#fff;border:1px solid #D8E6FF;border-radius:32px;box-shadow:0 24px 70px rgba(21,28,117,.12);overflow:hidden;">
          ...
          <div style="padding:28px;display:grid;grid-template-columns:1.1fr .9fr;gap:24px;">
            <div>
              ...
              ${waUrl(settings) ? `<a href="${esc(waUrl(settings))}" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;justify-content:center;gap:10px;padding:14px 18px;border-radius:16px;background:linear-gradient(135deg,#16A34A,#15803D);color:#fff;text-decoration:none;font-weight:900;font-size:13px;box-shadow:0 10px 24px rgba(22,163,74,.2);">◉ ${esc(settings.whatsapp_label || DEFAULTS.whatsapp_label)}</a>` : '<div style="font-size:12px;color:#64748B;">Kontak WhatsApp sedang disiapkan.</div>'}
            </div>
  ...
  async function validateImage(file) {
    ...
    if (!allowed.includes(file.type)) return { ok:false, message:'Format gambar harus JPG, PNG, atau WEBP.' };
    ...
    if (!dims?.w || !dims?.h) return { ok:false, message:'Gambar tidak dapat dibaca.' };
    return { ok:true, dims };
  }
  ...
  function adminMarkup(settings) {
    return `<div id="uc-admin-panel" class="space-y-5">
      <div class="rounded-3xl border border-blue-100 bg-gradient-to-br from-white to-blue-50/70 p-5">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div><div class="text-[9px] font-black uppercase tracking-[.12em] text-amber-600">SITE CONTROL</div><h2 class="mt-1 text-lg sm:text-xl font-black text-[#151c75]">Under Construction</h2><p class="mt-1 text-[10px] sm:text-xs text-slate-500">Mode maintenance terisolasi. Admin tetap dapat masuk dan mematikannya.</p></div>
          <label class="flex items-center gap-3 rounded-2xl border border-blue-100 bg-white px-4 py-3 cursor-pointer"><span class="text-xs font-black text-slate-700">Mode Under Construction</span><input id="uc-enabled" type="checkbox" ${settings.enabled?'checked':''} class="w-5 h-5 accent-[#151c75]"></label>
        </div>
      </div>
      <div class="grid xl:grid-cols-2 gap-4">
        <section class="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm space-y-3">
          <h3 class="text-sm font-black text-[#151c75]">Konten</h3>
      ...
      <div class="flex flex-wrap gap-2">
        <button id="uc-save" type="button" class="rounded-xl bg-[#151c75] px-4 py-2.5 text-[10px] font-extrabold text-white">Simpan Perubahan</button>
        <button id="uc-preview" type="button" class="rounded-xl border border-blue-100 bg-blue-50 px-4 py-2.5 text-[10px] font-extrabold text-[#151c75]">Preview Halaman</button>
      </div>
    </div>`;
  }
  ...
  async function renderAdmin() {
    ...
    area.innerHTML = adminMarkup(settings);
    const read = () => ({
    ...
    document.getElementById('uc-save')?.addEventListener('click', async () => {
      try { settings = await saveSettings(read()); renderAdmin(); toast('Pengaturan berhasil disimpan.','success'); } catch (e) { toast(e.message || 'Pengaturan belum tersimpan. Silakan coba lagi.','error'); }
    });
    document.getElementById('uc-preview')?.addEventListener('click', () => {
      const preview = read();
      const w = window.open('', '_blank', 'noopener,noreferrer');
      if (w) { w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Preview Under Construction</title></head><body></body></html>`); w.document.close(); w.document.body.innerHTML = ''; const holder = w.document.createElement('div'); w.document.body.appendChild(holder); w.document.documentElement.style.fontFamily='Inter,system-ui,sans-serif'; const old = document.body.innerHTML; document.body.innerHTML=''; renderPublic(preview); document.body.innerHTML=old; }
    });
    document.getElementById('uc-upload')?.addEventListener('click', async () => {
      const file = document.getElementById('uc-image-file')?.files?.[0];
      try { const url = await uploadImage(file); settings = await saveSettings({ image_url:url }); renderAdmin(); toast('Gambar berhasil diunggah.','success'); } catch (e) { toast(e.message || 'Upload gambar gagal.','error'); }
    });
    document.getElementById('uc-remove-image')?.addEventListener('click', async () => {
      try { await saveSettings({ image_url:'' }); renderAdmin(); toast('Gambar dihapus.','success'); } catch (e) { toast(e.message || 'Gambar belum bisa dihapus.','error'); }
    });
  }

  function installAdminTab() {
    if (!ADMIN) return;
    const findNav = () => [...document.querySelectorAll('button')].find(b => /Dapur Creator/.test(b.textContent || ''))?.parentElement;
    const install = () => {
      const nav = findNav();
      if (!nav || nav.querySelector('[data-uc-admin-tab]')) return;
      const active = false;
      const b = document.createElement('button');
      b.type='button'; b.dataset.ucAdminTab='1'; b.className='px-3.5 py-2 rounded-xl transition-all text-slate-700 hover:text-[#151c75]';
      b.innerHTML='<i class="fa-solid fa-person-digging mr-1"></i> Under Construction';
      b.addEventListener('click', () => { document.querySelectorAll('[data-uc-admin-tab]').forEach(x=>x.classList.remove('btn-brand-gradient','text-white')); b.classList.add('btn-brand-gradient','text-white'); renderAdmin().catch(e=>toast(e.message||'Panel gagal dimuat.','error')); });
      nav.appendChild(b);
    };
    install();
  ...
  async function boot() {
    try {
      if (ADMIN) { installAdminTab(); return; }
      if (!ROOT) return;
      const settings = await getSettings();
      if (!settings.enabled) return;
      renderPublic(settings);
    } catch (e) {
