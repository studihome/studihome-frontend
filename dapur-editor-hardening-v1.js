(()=>{
  ...
  async function resolveCreator(){
    ...
    if(resolved&&resolved.uid===uid&&resolved.slug===slug)return resolved.id;
  ...
  async function install(){
    ...
    if(typeof original.categories==='function')api.categories=async()=>original.categories(await resolveCreator());
    if(typeof original.service==='function')api.service=async()=>original.service(await resolveCreator());
    if(typeof original.portfolio==='function')api.portfolio=async()=>original.portfolio(await resolveCreator());
    api.__hardeningWrapped=true;
    window.DapurEditorHardening={resolveCreator,original};
    return true;
  }
  const start=()=>{if(!install())window.setTimeout(start,0)};
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
  window.addEventListener('hashchange',()=>{resolved=null});
})();