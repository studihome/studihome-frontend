(() => {
  ...
  function renderDetail(row, creator) {
    ...
    loadBundle(creator.id).then(bundle => {
      row.innerHTML = `
        <div class="p-4 border-t border-blue-50 bg-slate-50/50 space-y-4">
          ...
          <div class="grid xl:grid-cols-2 gap-3">
            ...
            <section class="rounded-2xl p-4 bg-white border border-slate-100"><div class="flex items-center justify-between mb-3"><h4 class="text-xs font-black text-[#151c75]">Status & Like</h4><strong class="text-sm text-[#151c75]">♥ ${creator.likeCount}</strong></div><div class="flex flex-wrap gap-2"><button type="button" data-action="minus" class="px-3 py-2 rounded-xl bg-white border border-slate-200 text-[10px] font-bold">− Like</button><button type="button" data-action="plus" class="px-3 py-2 rounded-xl bg-blue-50 border border-blue-100 text-[10px] font-bold text-[#151c75]">+ Like</button><span class="px-3 py-2 rounded-xl ${creator.is_verified ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'} text-[10px] font-bold">${creator.is_verified ? 'Verified' : 'Belum Verified'}</span><span class="px-3 py-2 rounded-xl ${creator.is_published ? 'bg-blue-50 text-[#151c75]' : 'bg-slate-100 text-slate-600'} text-[10px] font-bold">${creator.is_published ? 'Published' : 'Draft'}</span></div></section>
      ...
      row.querySelector('[data-action="plus"]').onclick = () => run(() => adjustLike(creator.id, 1), '1 like ditambahkan.', 'Like gagal ditambahkan.');
      row.querySelector('[data-action="minus"]').onclick = () => run(() => adjustLike(creator.id, -1), '1 like dikurangi.', 'Like gagal dikurangi.');
    }).catch(e => {
  ...
  function open() { if (isAdmin()) render(); }
  window.StudihomeAdminDapurCreatorV5 = { open, render };
})();
