(() => {
  ...
  async function adjustLike(id, delta) {
    ...
    if (!Number.isInteger(amount) || amount === 0) throw new Error('Jumlah penyesuaian Like belum valid.');
  ...
  function renderDetail(row, creator) {
    ...
    loadBundle(creator.id).then(bundle => {
      ...
      const runLikeAdjustment = async (direction) => {
        ...
        if (likeInput) likeInput.value = String(amount);
      ...
      row.querySelector('[data-action="plus"]').onclick = () => runLikeAdjustment('plus');
      row.querySelector('[data-action="minus"]').onclick = () => runLikeAdjustment('minus');
    }).catch(e => {
