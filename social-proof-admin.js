(() => {
  ...
  // ============================================================
  // Version: 4.0.0 (robust setInterval registration, no defer needed)
  // Date: 27 Aug 2026
  //
  // Registers App.admin.renderSocialProof() using setInterval polling
  // to guarantee the function is available before any button click.
  // ============================================================
    ...
    try {
      ...
      return orders.map(function(o) {
        return {
          ...
          email: (o.profiles && o.profiles.email) || '',
          product_title: (o.products && o.products.title) || 'Produk Premium',
          product_price: Number(o.total_amount || o.unit_price || 0),
  ...
  async function renderSocialProof() {
    ...
    if (!members.length) {
      ...
      var totalRevenue = 0;
      for (var i = 0; i < members.length; i++) totalRevenue += members[i].product_price;
      ...
      for (var j = 0; j < members.length; j++) {
        ...
        if (isInWidget) html += '<span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[8px] font-bold bg-blue-100 text-blue-700">WIDGET</span>';
        html += '</div>';
  ...
  // --- Bulletproof registration using setInterval ---
  // Polls every 100ms until App.admin exists, then attaches once.
  var registered = false;
  var pollInterval = setInterval(function() {
    if (registered) { clearInterval(pollInterval); return; }
    if (!window.App || !window.App.admin) return;

    // App.admin exists — attach our functions
