...
- NOT VERIFIED — full application/Preview browser parity for the complete A–I interaction set; the local real-Chromium binder smoke is narrower and must not be misreported as production E2E.\n- NOT VERIFIED — exact-head public Vercel Preview browser acceptance workflow result; Preview Browser run #1 failed in resolver parsing before ChromeDriver started, so it is automation failure evidence rather than application-regression evidence. Corrected resolver must PASS on a subsequent exact-head run before this item can become PASS.
