-- READ-ONLY fail-closed preflight for Issue #90.
-- Preserves the CURRENT EFFECTIVE media policy. It does not enable native video.
do $preflight$
declare
  v_drift integer;
  v_bad_rows integer;
begin
  ...
  if v_drift <> 0 then
    raise exception 'Preflight failed: legacy creator_portfolios media constraint fingerprint drifted (% mismatch rows).', v_drift;
  end if;
  ...
  if v_bad_rows <> 0 then
    raise exception 'Preflight failed: % existing creator_portfolios rows violate preserved effective policy.', v_bad_rows;
  end if;
end
$preflight$;
