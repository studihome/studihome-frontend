(()=> {
  ...
  const requestClose=e=>{if(state==='SAVING'){e.preventDefault();e.stopImmediatePropagation();toast('Sabar ya, lagi proses simpan!','info');return}if(state==='DIRTY'&&!window.confirm('Perubahan belum disimpan. Tutup form?')){e.preventDefault();e.stopImmediatePropagation();return}close()};
  get('de-close')?.addEventListener('click',requestClose);
  get('de-cancel')?.addEventListener('click',requestClose);
  save.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();doSubmit()});
  form.addEventListener('submit',e=>{e.preventDefault();doSubmit()});
  modal.addEventListener('click',async e=>{if(e.target===modal)return requestClose(e);const b=e.target.closest('[data-de-action]');if(!b)return;const action=b.dataset.deAction,id=b.dataset.deId,owner=b.dataset.deOwner;if(!id||!owner)return;try{if(action==='edit-service')return service(owner,id);if(action==='edit-portfolio')return portfolio(owner,id);if(action==='delete-service'){if(!confirm('Hapus layanan ini? Nggak bisa dibatalin lho!'))return;showLoading('Menghapus layanan…','Tunggu sebentar.');try{const r=await S().from('creator_services').delete().eq('id',id).eq('creator_id',owner);if(r.error)throw r.error;toast('Layanan dihapus. Bye bye! 👋','success');return service(owner)}finally{hideLoading()}}if(action==='delete-portfolio'){if(!confirm('Hapus karya ini? Nggak bisa dibatalin lho!'))return;showLoading('Menghapus karya…','Tunggu sebentar.');try{const r=await S().from('creator_portfolios').delete().eq('id',id).eq('creator_id',owner);if(r.error)throw r.error;toast('Karya dihapus. Bye bye! 👋','success');return portfolio(owner)}finally{hideLoading()}}}catch(error){console.error('[Dapur Action Error]',error);toast(error?.message||'Aksi belum bisa diproses.','error')}});
  transition('IDLE')}
  function initials(v){return String(v||'?').trim().slice(0,1).toUpperCase()}
  async function compressAvatar(file){if(!file||!/^image\/(jpeg|png|webp)$/.test(file.type))throw new Error('Foto harus JPG, PNG, atau WebP.');if(file.size>20*1024*1024)throw new Error('Ukuran foto maksimal 20 MB.');const src=URL.createObjectURL(file);try{const img=await new Promise((resolve,reject)=>{const i=new Image();i.onload=()=>resolve(i);i.onerror=()=>reject(new Error('Foto tidak dapat dibaca.'));i.src=src});const max=512,scale=Math.min(1,max/Math.max(img.naturalWidth,img.naturalHeight));const c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.naturalWidth*scale));c.height=Math.max(1,Math.round(img.naturalHeight*scale));const x=c.getContext('2d');x.drawImage(img,0,0,c.width,c.height);let q=.82;let blob=await new Promise(r=>c.toBlob(r,'image/webp',q));while(blob&&blob.size>450*1024&&q>.55){q-=.07;blob=await new Promise(r=>c.toBlob(r,'image/webp',q))}if(!blob||blob.size>500*1024)throw new Error('Foto masih terlalu besar setelah dikompres. Pilih foto lain.');return blob}finally{URL.revokeObjectURL(src)}}
  async function uploadAvatar(file,userId,profileId){const blob=await compressAvatar(file);const path=profileId?`${userId}/${profileId}/avatar.webp`:`${userId}/avatar.webp`;const st=S().storage.from('creator-media');const r=await st.upload(path,blob,{contentType:'image/webp',cacheControl:'3600',upsert:true});if(r.error)throw r.error;const u=st.getPublicUrl(path)?.data?.publicUrl;if(!u)throw new Error('URL foto profil tidak tersedia.');return `${u}?v=${Date.now()}`}
  function listBlock(title,note,rows,kind,owner){if(!rows?.length)return '';return `<section class="de-list"><div class="de-list-head"><span class="de-list-title">${esc(title)}</span><span class="de-list-note">${esc(note)}</span></div>${rows.map(r=>`<article class="de-item"><div class="de-item-main"><div class="de-item-title">${esc(r.title)}</div><div class="de-item-desc">${esc(r.description||'')}</div><div class="de-item-meta">${kind==='service'?`${r.is_active?'Aktif':'Disimpan'} · ${Number(r.delivery_days||0)} hari`:`${r.is_active?'Tampil':'Disimpan'} · ${esc(r.media_type||'link')}`}</div></div><div class="de-item-actions"><button type="button" class="de-mini" data-de-action="edit-${kind}" data-de-id="${esc(r.id)}" data-de-owner="${esc(owner)}">✏️ Edit</button><button type="button" class="de-mini danger" data-de-action="delete-${kind}" data-de-id="${esc(r.id)}" data-de-owner="${esc(owner)}">🗑️ Hapus</button></div></article>`).join('')}</section>`}
  async function profile(id){
    ...
    shell('✏️ Foyer · Profil Creator','Rapikan identitas utama Creator-mu. Username cuma bisa diganti sekali, foto otomatis dikompres biar hemat ruang.',`<div class="de-avatar"><div class="de-avatar-preview">${avatar?`<img src="${esc(avatar)}" alt="Foto profil">`:esc(initials(c.display_name))}</div><div class="de-file"><b>📸 Foto Profil</b><input name="avatar_file" type="file" accept="image/jpeg,image/png,image/webp"><small>JPG/PNG/WebP · maks 20 MB sebelum kompres · otomatis jadi ≤500 KB.</small></div></div>${field('username','👤 Username',c.username,'text',locked?'🔒 Username udah pernah diganti dan dikunci.':'3–40 karakter · huruf kecil, angka, atau tanda hubung (-)','autocomplete="username" pattern="(?:[a-z0-9]|-){3,40}" maxlength="40" '+(locked?'disabled':''))}${field('display_name','✨ Nama Tampilan',c.display_name,'text','Nama brand atau nama profesional kamu','required maxlength="80"')}${area('bio','📝 Bio Singkat',c.bio,'Ceritain dalam 1–3 kalimat siapa kamu dan apa yang kamu bantu.')}${field('whatsapp','📱 WhatsApp',c.whatsapp,'text','Contoh: 62812xxxx','inputmode="tel"')}${field('location','📍 Lokasi',c.location,'text','Kota atau area layananmu.')}`,
    async f=>{
      const username=String(f.get('username')||c.username).trim().toLowerCase();
      const p={display_name:String(f.get('display_name')||'').trim(),bio:String(f.get('bio')||'').trim(),whatsapp:String(f.get('whatsapp')||'').trim(),location:String(f.get('location')||'').trim()};
      if(!p.display_name)throw new Error('Nama tampilan wajib diisi.');
      if(!/^(?:[a-z0-9]|-){3,40}$/.test(username))throw new Error('Username harus 3–40 karakter: huruf kecil, angka, atau tanda hubung (-).');
      if(!locked&&username!==c.username){const v=await S().rpc('validate_creator_username',{p_username:username});if(v.error)throw v.error;const changed=await S().rpc('change_creator_username_once',{p_username:username});if(changed.error)throw changed.error}
      const file=f.get('avatar_file');if(file&&file.size)p.avatar_url=await uploadAvatar(file,c.user_id,id);
      let u;
      if(isOwner){u=await S().from('creator_profiles').update(p).eq('id',id).eq('user_id',c.user_id)}
      else{u=await S().from('creator_profiles').update(p).eq('id',id).eq('user_id',c.user_id)}
      if(u.error)throw u.error;
      // Verify update succeeded
      if(u.count!==undefined&&u.count===0)throw new Error('Update tidak berhasil. Profil mungkin sudah berubah.');
